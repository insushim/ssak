...
import { analyzeWriting } from "../utils/aiAPI";
import {
  PASSING_SCORE,
...
export async function cleanupOldFailedWritings(
  studentId,
  writings,
  passingScore = 70,
) {
  try {
    ...
    // 1시간 지난 미달성 글 필터링
    const oldFailedWritings = writings.filter(
      (w) =>
        !w.isDraft &&
        w.submittedAt &&
    ...
    devLog(
      `[자동 정리] 1시간 지난 미달성 글 ${oldFailedWritings.length}개 삭제됨`,
    );
...
export async function submitWriting(
  ...
  userData = null,
  testScoreMode = null,
  customTestScore = null,
  aiHelpHistory = [],
) {
  try {
    // 글자 수 기준 가져오기 (공통 normalizeGradeLevel 사용)
    const normalizedGrade = normalizeGradeLevel(writingData.gradeLevel);
    const standard = WORD_COUNT_STANDARDS[normalizedGrade] ||
      WORD_COUNT_STANDARDS["elementary-4"] || {
        min: 200,
        ideal: 350,
        max: 500,
      };
    ...
    if (aiHelpHistory && aiHelpHistory.length > 0) {
      const submittedText = writingData.content
        .replace(/\s/g, "")
        .toLowerCase();
      for (const aiText of aiHelpHistory) {
        ...
        if (
          submittedText.includes(cleanAiText) ||
          (cleanAiText.length > 20 &&
            submittedText.includes(cleanAiText.substring(0, 20)))
        ) {
          const similarity = cleanAiText.length / submittedText.length;
          if (similarity > 0.3) {
            // AI 텍스트가 제출글의 30% 이상 차지하면
            throw new Error(
              "AI 도움을 그대로 복사하면 안 됩니다. 참고만 하고 자신의 표현으로 다시 써주세요!",
            );
          }
        }
      }
    ...
    if (testScoreMode === "pass") {
      ...
      devLog(
        `[🧪 테스트] 도달 점수 모드: ${analysisResult.score} → ${newScore} (기준: ${minScore})`,
      );
    ...
    if (newScore < minScore) {
      ...
      const sameTopic = existingWritings.find(
        (w) =>
      ...
      if (sameTopic) {
        if (newScore <= sameTopic.score) {
          ...
          devLog(
            `[중복 방지] 기존 미제출글(${sameTopic.score}점)보다 낮거나 같음(${newScore}점) - 저장 안함`,
          );
          shouldSave = false;
        } else {
          // 기존 미제출글보다 점수가 높음 → 기존 글 삭제
          devLog(
            `[중복 방지] 기존 미제출글(${sameTopic.score}점)보다 높음(${newScore}점) - 기존 글 삭제`,
          );
          try {
    ...
    if (classCode) {
      ...
      // 🔧 newScore 사용 (테스트 모드에서 점수가 변경될 수 있음)
      await updateStudentRankingOnSubmit(
        classCode,
        studentId,
        newScore,
        userData,
    ...
    if (deletedOldWritingId) {
      await updateWritingSummary(
        studentId,
        { writingId: deletedOldWritingId },
        "delete",
      );
    }
...
export async function getStudentStats(studentId, forceRefresh = false) {
  try {
    ...
    if (!forceRefresh) {
      ...
      const lsData = loadFromLocalStorage(
        `stats_${studentId}`,
        CACHE_TTL.studentStats,
      );
      if (lsData) {
        cache.studentStats.set(studentId, {
          data: lsData,
          timestamp: Date.now(),
        });
...
// (안전장치로 폴백 로직 유지)
async function getAllClassWritingsBatch(
  classCode,
  studentIds = [],
  forTeacher = false,
) {
...
export async function getClassWritingsSummary(classCode, forceRefresh = false) {
  try {
    ...
    if (
      !forceRefresh &&
      cached &&
      isCacheValid(cached.timestamp, CACHE_TTL.classWritings)
    ) {
      // 🔇 디버그 로그 감소
      return cached.data;
    }
...
export async function getClassWritings(
  classCode,
  forceRefresh = false,
  forTeacher = true,
) {
  try {
    ...
    if (
      !forceRefresh &&
      cachedWritings &&
      isCacheValid(cachedWritings.timestamp, CACHE_TTL.classWritings)
    ) {
      // 🔇 디버그 로그 감소
    ...
    if (
      !forceRefresh &&
      cachedClass &&
      isCacheValid(cachedClass.timestamp, CACHE_TTL.classData)
    ) {
    ...
    // 🔧 forTeacher=true면 classCode가 없는 기존 글도 studentId로 폴백 조회
    const allWritings = await getAllClassWritingsBatch(
      classCode,
      studentIds,
      forTeacher,
    );
    ...
    const sortedWritings = allWritingsWithNicknames.sort(
...
export async function getClassRanking(
  ...
  try {
    ...
    if (
      savedRanking &&
      savedRanking.periodKey === periodKey &&
      savedRanking.data
    ) {
      // 🔇 디버그 로그 감소
...
export async function updateStudentRankingOnSubmit(
  classCode,
  studentId,
  score,
  userData,
  ...
  try {
    ...
    for (const period of ["weekly", "monthly"]) {
      ...
      // 기존 랭킹에서 해당 학생 찾기
      const studentIndex = rankingData.findIndex(
      ...
      if (studentIndex >= 0) {
        ...
        const newTotalScore =
          student.averageScore * (student.submissionCount - 1) + score;
        student.averageScore = Math.round(
          newTotalScore / student.submissionCount,
        );
        ...
        student.rankingScore =
          student.averageScore * 3 + student.passCount * 20;
...
export async function getStudentRankingOptimized(
  classCode,
  studentId,
  period = "weekly",
  options = {},
...
export async function getStudentGrowthData(studentId) {
  try {
    ...
    const growthData = Object.values(dailyData).map((day) => ({
      date: day.date,
      displayDate: new Date(day.date).toLocaleDateString("ko-KR", {
        month: "short",
        day: "numeric",
      }),
...
export function getWritingSummaryFromUserData(userData) {
  ...
  return [...userData.writingSummary].sort(
    (a, b) =>
...
export async function updateWritingSummary(
  studentId,
  writingData,
  action = "add",
) {
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (!userDoc.exists()) {
        console.error(
          "[writingSummary] users 문서가 존재하지 않음:",
          studentId,
        );
      ...
      if (action === "add" || action === "update") {
        ...
        // undefined가 아닌 값만 추가
        if (writingData.submittedAt)
          newEntry.submittedAt = writingData.submittedAt;
        if (writingData.createdAt) newEntry.createdAt = writingData.createdAt;
        if (
          writingData.minScore !== undefined &&
          writingData.minScore !== null
        ) {
